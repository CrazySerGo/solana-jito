...
fn allnodes_poh_resolve_cpu_core(shred_version: u16, ledger_path: &Path) -> Option<usize> {
    ...
    let filename = ledger_path.join(format!("poh-{cpuid:016x}.bin"));
    let saved = std::fs::read(&filename)
        .ok()
    ...
    let benchmark = match saved {
        ...
        None => {
            ...
            std::fs::write(&filename, &encoded).ok();
...
pub fn rpc_bootstrap(
    ...
    if validator_config.poh_pinned_cpu_core.is_none() {
        validator_config.poh_pinned_cpu_core =
            allnodes_poh_resolve_cpu_core(expected_shred_version, ledger_path);
    }
